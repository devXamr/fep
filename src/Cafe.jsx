...
export default function Cafe(){
    ...
    return <div>
        <div className={`${confirmClicked && 'overflow-hidden fixed'}`}>
        <div className='font-black text-5xl text-left mb-7'>Desserts</div>
        <SingleMenuItem category='Waffle' name='Waffle with Berries' image={waffle_mob} price='6.50' addFunc={addToCart} subFunc={removeFromCart} count={cart[0].qty}/>
        <SingleMenuItem category='Creme Brulee' name='Vanilla Bean Creme Brulee' image={creme} price='7.00' addFunc={addToCart} subFunc={removeFromCart} count={cart[1].qty}/>
        <SingleMenuItem category='Macaron' name='Macaron Mix of Five' image={macaron} price='8.00' addFunc={addToCart} subFunc={removeFromCart} count={cart[2].qty}/>
        <SingleMenuItem category='Tiramisu' name='Classic Tiramisu' image={tiramisu} price='5.50' addFunc={addToCart} subFunc={removeFromCart} count={cart[3].qty}/>
        <SingleMenuItem category='Baklava' name='Pistachio Baklava' image={baklava} price='4.00' addFunc={addToCart} subFunc={removeFromCart} count={cart[4].qty}/>
        <SingleMenuItem category='Pie' name='Lemon Meringue Pie' image={meringue} price='5.00' addFunc={addToCart} subFunc={removeFromCart} count={cart[5].qty}/>
        <SingleMenuItem category='Cake' name='Red Velvet Cake' image={cake} price='4.50' addFunc={addToCart} subFunc={removeFromCart} count={cart[6].qty}/>
        <SingleMenuItem category='Brownie' name='Salted Caramel Brownie' image={brownie} price='5.50' addFunc={addToCart} subFunc={removeFromCart} count={cart[7].qty}/>
        <SingleMenuItem category='Panna Cotta' name='Vanilla Panna Cotta' image={cotta} price='6.50' addFunc={addToCart} subFunc={removeFromCart} count={cart[8].qty}/>
        <div className='rounded-lg shadow-md bg-white px-6 py-9'>
            ...
            {cart.filter(i => i.qty > 0).length === 0 && <div><img src={empty} className='mx-auto mb-3'/>
                <div>Your added items will appear here</div></div>}

            {cart.filter(i => i.qty > 0).map(e => <SingleCartItem rate={e.price} itemName={e.name} quantity={e.qty} totalItemPrice={e.price * e.qty} delFunc={deleterFunc}/>)}
            ...
            { totalQty > 0 && <div>
                ...
                <img src={neutral}/>
                <div className='ml-2'>This is a <span className='font-bold'>carbon-neutral</span> delivery</div>
                </div>
                <div className='bg-amber-800 text-white text-xl py-4 px-4 rounded-full' onClick={() => setConfirmClicked(true)}>Confirm Order</div>
            </div>}
        ...
        {confirmClicked && <div className='inset-0 fixed bg-black bg-opacity-30'>
            ...
            <div className='bg-white mt-24 h-full rounded-t-xl z-30'>
                ...
                <div className='bg-amber-800 text-white text-xl py-4 px-4 rounded-full mx-6 mt-8' onClick={newOrder}>Start New Order</div>
            </div>
